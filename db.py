...
class Database:
    ...
    def schema_ready(self) -> bool:
        """Return True when the core no-code dashboard schema already exists.

        This check is intentionally read-only and fast. Re-running the full DDL
        migration on every web-service restart can wait behind a long export or
        quality query and hit Supabase's statement timeout.
        """
        required = (
            "instruments",
            "market_bars",
        ...
        with self.connection() as conn:
            with conn.cursor() as cur:
                ...
                return int(cur.fetchone()[0]) == len(required)
